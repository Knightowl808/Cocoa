...
Quantile output     & Empirical      & Closed-form  & None           & Direct         & Direct         \\
...
simplex algorithm \parencite{koenker1987algorithm}.
...
Calibration is tested formally with the likelihood-ratio framework of
\textcite{christoffersen1998evaluating}. The unconditional coverage test
($LR_{uc}$, $\chi^2_1$) tests whether the violation frequency equals the
nominal rate $1 - \tau$; the independence test ($LR_{ind}$, $\chi^2_1$)
tests whether violations arrive independently rather than in clusters,
using a first-order Markov alternative; and the conditional coverage test
($LR_{cc} = LR_{uc} + LR_{ind}$, $\chi^2_2$) combines both properties.
One complication arises from the daily rolling design: at horizons
$h > 1$, consecutive forecasts target overlapping $h$-day windows, so
consecutive violation indicators are dependent by construction and the
independence test would reject mechanically. We therefore compute the
test statistics on the non-overlapping subsequence of every $h$-th
forecast, while reporting the empirical violation rate over all
forecasts. At the longest horizons this leaves few non-overlapping
observations (28 at twelve months), so the tests lose power and the raw
violation rate becomes the more informative diagnostic.
...
parameters on the 2,520 most recent observations whose $h$-day target
windows are fully realized by $t$ --- feature dates
$s \in \{t-h-2519, \ldots, t-h\}$, so that no training target extends
beyond the forecast origin --- generate forecasts, and roll forward by one
day. The window rolls \emph{daily} to maximize the number of
...
check re-estimates HAR-X-QR with the Ni\~no~3.4 index lagged 0, 1, 3, 6, and~12
