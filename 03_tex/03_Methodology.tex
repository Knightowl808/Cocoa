...
Gaussian closed-form (Eq.~\ref{eq:garch-quantile}); Check fn.\ = check-function (quantile) loss;
...
bonatoNinoNinaForecastability2023} and is fully causal: the Ni\~no~3.4
...
\paragraph{Robustness diagnostics for the ENSO signal.}
...
re-evaluates the gain on a pre-crisis subsample ending 8~February~2024,
estimated via a sup-Wald structural break test \parencite{andrews1993tests},
which excludes the extreme-price phase of the 2022--2024 supply crisis:
a structural relationship should survive the exclusion of any single episode. Second, a \emph{lag-structure}
check re-estimates HAR-X-QR with the Ni\~no~3.4 index lagged 0, 1, 3, and~6
months and inspects the cross-correlogram between ENSO and cocoa volatility.
Under the agronomic-transmission hypothesis the predictive content should
strengthen at lags of 6--12 months, matching the window over which
ENSO-driven rainfall anomalies are hypothesised to affect harvests; this is
the lag horizon at which ENSO is found to improve commodity volatility
forecasts in the oil and agricultural literature
\parencite{bouriNinoForecastabilityOilprice2021,
bonatoNinoNinaForecastability2023}. The cross-correlogram follows standard
cross-correlation diagnostics for lead--lag relationships between time series
\parencite{tsay2010analysis}. Third, a \emph{cross-commodity} check applies
