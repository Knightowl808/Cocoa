...
$V_t$. Trading volume is collected but not used in the volatility models.
...
The daily data contain no missing values for trading days; non-trading
days are excluded. Overnight returns spanning weekends or holidays are
...
three-month running mean exceeds $+0.5$°C (falls below $-0.5$°C) for
at least five consecutive months. In the regression models
(Equation~\ref{eq:har-x-qr}), $\text{ENSO}_t$ enters as the continuous
Ni\~no~3.4 value rather than a categorical regime indicator, preserving
the full variation in anomaly magnitude.
...
and Phillips--Perron (PP) unit root tests for the three key series (reported
in Table~\ref{tab:stationarity}, Appendix~\ref{app:supplementary}). Both
...
volatility proxy. That validation, reported in
Table~\ref{tab:proxy-validation} of Appendix~\ref{app:supplementary}, confirms
that the Yang-Zhang estimator tracks intraday realized volatility closely,
with the correlation rising to 0.92 at the monthly frequency relevant to the
forecasting exercise. The next section presents the empirical results from the
