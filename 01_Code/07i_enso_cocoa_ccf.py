...
The figure is purely descriptive (no model, no look-ahead) and is placed
in the Data section. A single full-sample panel: the question of whether
any ENSO signal is structural or episodic is answered separately by the
expanding-window Delta_QL diagnostics (07k/07l), so no arbitrary
pre-crisis/crisis split is imposed here.

Outputs:
  02_Output/figures/35_enso_cocoa_ccf.png
  02_Output/tables/enso_cocoa_ccf.csv

Diagnostic only --- not used as a thesis figure (cut from the RQ2 section).
"""
...
def load_monthly_data():
    ...
    Returns aligned (enso, vol) arrays over the full sample.
    """
...
if __name__ == "__main__":
    print("=" * 60)
    print("ENSO–Cocoa CCF: full sample")
    print("=" * 60)

    enso_full, vol_full   = load_monthly_data()
    ...
    print(f"Significant lags (|r| > CI): full={n_sig_f}")
    ...
    # Plot — single full-sample panel
    fig, ax = plt.subplots(1, 1, figsize=(6.5, 2.8))

    plot_ccf(ax, lags_f, corrs_f, ci_f, "", len(enso_full))
    ax.set_xlabel("Lag (months) — positive: ENSO leads volatility")

    plt.tight_layout()

    out_png = os.path.join(FIG_DIR, "35_enso_cocoa_ccf.png")
    fig.savefig(out_png)
