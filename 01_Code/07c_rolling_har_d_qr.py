"""
07c_rolling_har_d_qr.py
=======================
Rolling-window HAR-D-QR: HAR-QR augmented with a crisis regime dummy.
...
where D_t = 1 if Date is within the 2022–2024 cocoa supply crisis period,
      D_t = 0 otherwise.

Purpose — ENSO regime test (from 17.03.2026 meeting with Dr. Bleher):
  Johannes raised the concern that ENSO's improvement at long horizons may
  be spurious, driven by coincidental overlap with the 2022–2024 crisis
  (CSSVD virus + weather shocks). This script provides the "dummy-only"
  counterfactual. By comparing:
    (A) HAR-D-QR   (this script)
    (B) HAR-X-QR   (07b — ENSO only)
    (C) HAR-XD-QR  (07d — ENSO + dummy)
  we can isolate ENSO's incremental contribution over and above regime effects.

Crisis dummy definition:
  D_t = 1  for 2022-01-01 <= Date <= 2024-12-31
  D_t = 0  otherwise
  Rationale: The 2022–2024 crisis is a predetermined, externally documented
  event (not constructed from in-sample volatility), so there is no
  look-ahead bias. The start/end dates are consistent with the crisis-period
  analyses in 10b_crisis_zoom.py.
...
Output:
  00_Data/processed/har_d_qr_forecasts_{h}.csv  — per-horizon forecasts
  02_Output/tables/har_d_qr_quantile_loss.csv   — QL across all (h, tau)
  02_Output/tables/har_d_qr_coverage.csv        — tau=0.95 coverage
  02_Output/figures/27_har_d_qr_fanchart_{h}.png
"""
...
WINDOW = 2520          # ~10 years of training data
REESTIMATE_EVERY = 5   # re-estimate QR every 5 trading days

# Crisis period: 2022–2024 cocoa supply crisis (CSSVD + weather shocks)
...
# ---------------------------------------------------------------------------
def load_data():
    """Load the HAR dataset and add the crisis regime dummy."""
...
def add_crisis_dummy(df):
    ...
    The dummy is fully determined by date, contains no NaNs, and introduces
    no look-ahead bias since the crisis period boundaries are predetermined
    and externally documented.
    """
    df = df.copy()
    df["crisis_dummy"] = (
...
# Rolling HAR-D-QR
# ---------------------------------------------------------------------------
def rolling_har_d_qr(df, horizon_key, horizon_label):
    """
    Rolling-window HAR-D-QR for a single horizon, all quantiles.

    Features: [sigma_d, sigma_w, sigma_m, crisis_dummy]
    The crisis dummy is treated identically to any other regressor —
    it enters the quantile regression with its own tau-specific coefficient.

    Returns DataFrame with columns: Date, actual, q05, q25, q50, q75, q95
    """
    target_col   = f"target_{horizon_label}"
    feature_cols = ["sigma_d", "sigma_w", "sigma_m", "crisis_dummy"]

    # 'enso' is NOT a regressor here; included in the dropna only to align
    # the evaluation sample with the ENSO-augmented models (07b/07d).
    ...
    last_estimated = -REESTIMATE_EVERY  # force estimation on first step
    ...
    h = horizon_key
    for t in tqdm(range(WINDOW + h, n), desc=f"  HAR-D-QR h={horizon_label}", leave=True):
        ...
        if t - last_estimated >= REESTIMATE_EVERY:
            ...
            for tau in QUANTILES:
                try:
                    ...
                    pass  # keep previous params if solver fails
        ...
        for tau in QUANTILES:
            ...
            q_forecasts[tau].append(max(y_hat, 1e-10))  # non-negativity floor
...
def plot_fan_chart(results, horizon_label):
    ...
    ax.set_title(f"HAR-D-QR Fan Chart (crisis dummy) — Horizon {horizon_label}")
    ax.set_xlabel("Date")
    ax.set_ylabel("Average Daily Volatility")
    ax.legend(loc="upper left")
    fig.tight_layout()

    fname = os.path.join(FIG_DIR, f"27_har_d_qr_fanchart_{horizon_label}.png")
    fig.savefig(fname)
    plt.close(fig)
    print(f"  Saved: 27_har_d_qr_fanchart_{horizon_label}.png")
...
if __name__ == "__main__":
    print("=" * 60)
    print("HAR-D-QR — HAR-QR with 2022–2024 Crisis Regime Dummy")
    print("=" * 60)
    print("\nModel specification:")
    print("  Q_tau(sigma_{t+h}) = b0(tau) + b_d(tau)*sigma_d + b_w(tau)*sigma_w")
    print("                       + b_m(tau)*sigma_m + b_D(tau)*D_t")
    print(f"\nCrisis period: {CRISIS_START.date()} to {CRISIS_END.date()}")
    print("Purpose: regime dummy counterfactual for ENSO regime test")
    print("\n" + "=" * 60 + "\n")
    ...
    for h, label in HORIZONS.items():
        ...
        res = rolling_har_d_qr(df, h, label)
        ...
        if res is not None and len(res) > 100:
            ...
            res.to_csv(
                os.path.join(PROC_DIR, f"har_d_qr_forecasts_{label}.csv"),
                index=False
            )
    ...
    if all_results:
        ...
        loss_df.to_csv(
            os.path.join(TBL_DIR, "har_d_qr_quantile_loss.csv"), index=False
        )
        ...
        coverage.to_csv(
            os.path.join(TBL_DIR, "har_d_qr_coverage.csv"), index=False
        )
